const Hero = () => {
  return (
    <>
        <div className="w-full h-[auto] bg-white flex flex-col justify-center items-center gap-0
                       md:w-full md:flex md:flex-row md:justify-center md:items-center md:gap-20">

            <div className="w-[80%] h-[45%] text-black
                            md:w-[30%] md:h-[auto]">

                <h1 className="text-[48px] font-bold leading-none md:text-[62px] md:font-bold">Business <br /> & Finance</h1>

                <span className="inline-block w-[100%] h-[2px] bg-cyan-300
                                md:w-[100%]"></span>


                <p className="text-[28px] font-bold md:text-32px">&quot;I want to understand how finance works, and be part of it&quot;</p>
            </div>

            <div className="w-[80%] h-[45%] flex justify-center items-center
                            md:w-[45%] md:h-[90%]"> 


                <img className="w-[100%] h-[100%]" src="../courses/Business.png" alt="Business & Finance image" />
            </div>    
 
        </div> 

        <div className="w-full h-[40px] md:h-[50px] flex flex-row">
            <div className="w-[10%] h-full bg-[#0C1C44] relative overflow-hidden">
                <div className="absolute top-0 right-0 w-full h-full bg-white rounded-bl-full"></div>
            </div>
            
            <div className="w-[80%] h-full bg-white">

            </div>

            <div className="w-[10%] h-full bg-[#0C1C44] relative overflow-hidden">
                <div className="absolute top-0 left-0 w-full h-full bg-white rounded-br-full"></div>
            </div>
        </div>
    </>
  )
}

export default Hero
